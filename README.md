...
[Скачать](#скачать) · [Возможности](#возможности) · [Что не работает](#что-не-работает) · [Как пользоваться](#как-пользоваться) · [Сборка](#сборка) · [Структура](#структура-проекта)
...
Честный список ограничений — чтобы не гадать, почему сервер не подключается.

**Протоколы, которых нет в Xray:** TUIC, AnyTLS, ShadowTLS. Такие серверы остаются в списке,
но помечаются как неподдерживаемые.

**Транспорты, удалённые из Xray:** HTTP/2 и QUIC. Тоже помечаются.

**REALITY работает только поверх TCP, XHTTP и gRPC** — так устроено ядро. Связка REALITY с
WebSocket, mKCP или HTTP Upgrade будет отклонена.

**Правила маршрутизации по имени приложения** не переносятся: ядро не умеет сопоставлять uid
Android с пакетом. Режим «прокси только для выбранных приложений» при этом работает — он
живёт на уровне системного VPN и ядра не касается.

**Раздача через точку доступа** невозможна: Android обрабатывает её ниже уровня VPN, и
трафик подключённых устройств физически не проходит через туннель приложения. Рабочий путь —
локальный прокси, см. выше.

...
func NewMethod(ctx context.Context, methodName string, options C.MethodOptions) (C.Method, error) {
	...
	if options.Password != "" {
		...
		pskList = make([][]byte, 0, len(keyStrList))
		for _, keyStr := range keyStrList {
			...
			if err != nil {
				kb, err = base64.URLEncoding.DecodeString(keyStr)
			}
			if err != nil {
				kb, err = base64.RawStdEncoding.DecodeString(keyStr)
			}
			if err != nil {
				kb, err = base64.RawURLEncoding.DecodeString(keyStr)
			}
			if err == nil && len(kb) != 16 && len(kb) != 32 {
				if len(keyStr) == 16 || len(keyStr) == 32 {
					kb = []byte(keyStr)
					err = nil
				}
			}
